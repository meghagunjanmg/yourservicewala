...
import 'package:yourservicewala/models/Referaldetails.dart';
...
class _ReferalsDetailState extends State<ReferalsDetailWidget> {
  ...
  getAPiData() async {
    List<List<String>> data11 = [];
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    String? phoneNumber = await prefs.getString("userMobileNo");

    response = await API.GetReferalDetails(context, phoneNumber!);
      for(int i=0;i<response!.allresults!.length;i++){
        List<String> arr = [
          (i+1).toString(),
          response!.allresults![i].name.toString(),
          response!.allresults![i].mobileNo.toString(),
          response!.allresults![i].emailID.toString(),
          response!.allresults![i].userAddress.toString(),
          response!.allresults![i].joinDate.toString(),
          response!.allresults![i].status.toString(),
          response!.allresults![i].description.toString(),
        ];
        if(response!.allresults![i].status.toString() == "1")
          {
            data11.add(arr);
          }
      }

    data11.insert(0,columns);
